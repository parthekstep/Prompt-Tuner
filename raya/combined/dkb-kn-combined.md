...
**Opening is gated on `${call_direction}` (auto-injected; see Input Variables). Evaluate it first, before the Phase Entry Rule. If `${call_direction}` is `outbound`, use the OUTBOUND opening block below and ignore the INBOUND block. If `${call_direction}` is `inbound`, skip the OUTBOUND block and use the INBOUND opening block that follows it. Everything after this section is shared and runs regardless of direction.**

---

**■ OUTBOUND opening — use only when `${call_direction}` is `outbound`:**
...
**■ INBOUND opening — use only when `${call_direction}` is `inbound`:**
