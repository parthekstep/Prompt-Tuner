...
**Step 7 — deploy.** API PATCH, name-guarded, read-back-verified, recorded to `raya/deploy-history.md`. `scripts/raya_deploy.py deploy <target>` does this. Never edit the live agent in the Raya console (it clobbers console-only content). Confirm all changed targets are `in sync` afterward. A deploy is NOT the finish line: the fix is not done until the **three testing tiers** pass (repo `CLAUDE.md` → "The three testing tiers"): **Tier 1** a post-deploy voice-test/transcript confirms the corrected behavior; **Tier 2** blast-radius regression confirms nothing adjacent broke; **Tier 3** the standing daily suite catches longer-tail drift. Run tiers 1–2 **on every affected variant separately — never extrapolate from one language/direction** ("works for Hindi so Kannada is fine" is a recipe for disaster). `Fixed for UAT` means deployed + ready-to-test, never *confirmed*.
