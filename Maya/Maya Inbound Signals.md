...
]** The list below is the **real, current** UP-graduate inventory (pulled from the live agent, which the team maintains directly). Do not present any job not listed here. Keep the `job_id`s exact — they are passed to `apply_job`. Same presentation rules, field rules, and hallucination guard apply as in the outbound version. When the team updates jobs on the live console, re-reconcile before editing.
>
> **[SIGNALS DATA DEPENDENCY — action required before this bot can apply]** On the Signals backend, `apply_job.target_item.item_id` must be a real Signals **`job_posting_1.0`** item_id. The `job_id`s below are the **current console ids** and are kept **AS-IS**; they must be **repointed to Signals `job_posting_1.0` item_ids separately** (a data dependency, outside this prompt — see the migration guide §E). Until they are repointed, `apply_job` will 404. Do not strip or reformat any `job_id` (full hyphenated 8-4-4-4-12 UUID, all four hyphens intact).
